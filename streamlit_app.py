...
English_Model = tf.keras.models.load_model('English_Bidirectional_LSTM.h5')
...
with st.sidebar:
    ...
    Number = st.slider('Insert Number Of Words', min_value=1, max_value=200) 
    ...
    if Language == 'English':   
        if st.button('Generate Text'):
           for _ in range(Number): 
                    ...
                    token_list = tf.keras.utils.pad_sequences([token_list], maxlen=17, padding='pre') 
